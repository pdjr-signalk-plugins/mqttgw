        ...
        start: function (options) {
            ...
            plugin.options.publication.paths = plugin.options.publication.paths.reduce((a, path) => {
                if (path.path) {
                    ...
                    app.setPluginError("dropping publication with missing 'path' property");
                return (a);
            }, []);
            plugin.options.subscription.topics = plugin.options.subscription.topics.reduce((a, topic) => {
                if (topic.topic) {
                    ...
                    app.setPluginError("dropping subscription with missing 'topic' property");
                return (a);
            }, []);
            app.debug(`using configuration: ${JSON.stringify(plugin.options, null, 2)}`);
            const client = (0, mqtt_1.connect)(plugin.options.broker.mqttBrokerUrl, {
            ...
            function startSending(publicationoptions, client) {
                ...
                publicationoptions.paths.forEach((path) => {
                    ...
                    if (path.meta)
                        app.debug(`publishing topic '${path.topic}/meta'`);
